...
Important: we have **not** proven that the Hugging Face cache contract is the only cause. It is a real and repeatedly observed difference, but other causes remain plausible. The latest agreed diagnostic is to compare step-1 decode cache state between `originalQevict` and current `src` with identical logging.

The current strongest hypothesis is:

> The sticky scoring/eviction math may be equivalent, but the effective generation protocol is not. Differences in cache object semantics, physical cache length, true position tracking, prompt/question compression schedule, and recurrent decode eviction can all change generation quality even when the selection formula is the same.
...
| File | Change |
|---|---|
| `src/models/attention/module.py` | Added `[STEP1-CACHE current]` diagnostic; earlier work added cache dispatch and StickyDynamicLayer writeback |
| `originalQevict/sticky_llama_attention.py` | Added matching `[STEP1-CACHE original]` diagnostic and `_dbg_count` |
...
## Key Numerical Clue From Recent Logs
...
## The Latest Diagnostic Added

We added comparable step-1 logs in both repos.
...
Printed exactly once for layer 0 at decode step 1:
...
|---|---|
| Both have same `phys_before`, `concat_len`, `pos_ids`, `global_tc` | Cache contract/position is less likely to explain score gap; inspect eviction/Q-cache values |
| Original has near-full prompt `phys_before`, current has compressed `phys_before` | The two runs are not evaluating the same effective compression |
| Current has wrong `pos_ids` or `global_tc` | StickyDynamicLayer or prep override still wrong |
| Original receives `cache_type=None` or no step-1 cache | Original score may not be using the assumed path |
| Original uses tuple cache and current uses `DynamicCache` but lengths/positions match | Cache object API differs, but semantics may be equivalent enough; move to tensor content comparison |
...
- "OriginalQevict definitely used near-full cache."
...
- Step-1 physical cache length/position must now be compared between original and current.

## Open Hypotheses

| ID | Hypothesis | Why plausible | How to test |
|---|---|---|---|
| H1 | Original and current have different step-1 physical cache length | K-norm differs by roughly 2x in logs | Compare `[STEP1-CACHE original/current]` |
...
| H5 | RoPE absolute-position semantics still mismatched somewhere | Compression with RoPE is fragile | Log and compare `position_ids`, true/global counter, physical lengths |
...
Run the same small LCC diagnostic in both:
...
Then compare the first `[STEP1-CACHE ...]` line for sample 0 and sample 2.
...
If those differ, the next fix should target the semantic mismatch.

If those match, the next diagnostic should compare tensor content:
...
```text
[STEP1-CACHE original] cache_type=tuple q_len=1 pos_ids=[1737] global_tc=1737 phys_before=420 concat_len=421 use_cache=True
[STEP1-CACHE current] cache_type=DynamicCache q_len=1 pos_ids=[1737] global_tc=1737 phys_before=420 concat_len=421 use_cache=True
```

This would mean:

- Step-1 cache length and true position match.
- Move away from cache API as primary explanation.
- Compare selected windows and tensor contents.

Example mismatch:

```text
[STEP1-CACHE original] ... phys_before=1675 concat_len=1676 ...
[STEP1-CACHE current] ... phys_before=420 concat_len=421 ...
```

This would mean:

- Original is not running the same effective compression.
- Score comparison is not apples-to-apples until original's cache path is understood.

Another mismatch:

```text
[STEP1-CACHE current] ... pos_ids=[420] global_tc=1737 ...
```

This would mean:

- Position path is still wrong in current.
- Revisit `StickyDynamicLayer`, `prepare_inputs_for_generation`, and HF model-level `cache_position`.
...
- The newest step-1 logs are the next decisive measurement.
